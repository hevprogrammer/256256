  ...
  fetch("http://localhost:5000/api/members", { mode: "cors" })
    ...
    .then((data) => {
      ...
      data.forEach((element) => {
        html += `<li>${element.first_name} ${element.last_name} <a href="javascript:void(0)" onClick="deleteMember(${element.id})"> <i class="fa fa-trash-o"></i>
         </a></a></li> <a href="javascript:void(0)" onClick="editMember(${element.id})"> Edit </a>`;
      });
...
function editMember(id) {
  fetch(`http://localhost:5000/api/members/${id}`)
    ...
    .then((data) => {
      ...
      document.querySelector("#id_address").value = data[0].ip_address;
